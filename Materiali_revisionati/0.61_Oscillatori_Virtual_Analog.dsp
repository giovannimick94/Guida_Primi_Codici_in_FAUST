...
// con "< 0". Della rampa passa solo il segnale minore di 0.5
// mantenendo la sua curva originale grazie alla moltiplicazione.

negative_parttr(f) = (scale_phasortr(f) < 0) * scale_phasortr(f);

// con "> 0". Della rampa passa solo il segnale maggiore di 0.5
// mantenendo la sua curva originale grazie alla moltiplicazione.

positive_parttr(f) = (scale_phasortr(f) > 0) * scale_phasortr(f);

// riportiamo in negativo la parte positiva facendo in modo che
// la seconda metà del segnale non vada da 0. a 0.5
// ma da 0. a -0.5

transposed_positivetr(f) = positive_parttr(f) * -1;

//sommiamo le due metà per riformare il segnale:
...
// dunque per rimetterlo in fase aggiungiamo 0.25 così da
// farlo oscillare tra -0.25 e +0.25

triangle_negativetr(f) = full_triangletr(f) + 0.25;

// Infine controllo l'ampiezza con il parametro "a".
...
// *-1 inverte il segnale e +0.5 reimposta la fase eliminando l'offset.

saw(f,a) = (phase(f)*-1+0.5)*a;

// ONDA QUADRA 
// A partire dal fasore generato impostiamo una condizione:
// deve passare della rampa solo il segnale maggiore di 0.5 divenendo 1.
...
process = square(200,0.2)+saw(250.5,0.3)+tri(300,0.2) <:_,_;
